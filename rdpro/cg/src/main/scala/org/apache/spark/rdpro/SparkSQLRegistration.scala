...
import edu.school.org.lab.rdpro.sql._
import org.apache.spark.rdpro.sql.{EnvelopeDataType, GeometryDataType}
...
object SparkSQLRegistration {
  ...
  def registerUDF(sparkSession: SparkSession): Unit = {
    functions.foreach(f => {
      val name = getFunctionName(f)
      if (!sparkSession.sessionState.functionRegistry.functionExists(FunctionIdentifier(name)))
        sparkSession.sessionState.functionRegistry.createOrReplaceTempFunction(getFunctionName(f), f, "scala_udf")
    })
