 ...
 */
package edu.school.org.lab.rdpro.geolite

import com.esotericsoftware.kryo.io.{Input, Output}
import com.esotericsoftware.kryo.{Kryo, KryoSerializable}
import edu.school.org.lab.rdpro.geolite.Feature.{readType, readValue, writeType, writeValue}
import edu.school.org.lab.rdpro.util.{BitArray, KryoInputToObjectInput, KryoOutputToObjectOutput}
import org.apache.spark.rdpro.sql.GeometryDataType
...
object Feature {
   ...
   * @return
   */
  private def inferSchema(values: Array[Any]): StructType = StructType(values.map(v => StructField(null, detectType(v))))
  ...
  def create(row: Row, geometry: Geometry): Feature =
    if (row == null) {
      ...
      if (iGeom == -1) {
        ...
        val values: Array[Any] = Row.unapplySeq(row).get.toArray
        if (geometry != null)
          values(iGeom) = geometry
        new Feature(values, rowSchema)
      }
    }
