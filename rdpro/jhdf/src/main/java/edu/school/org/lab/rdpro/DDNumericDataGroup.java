 ...
 */
public class
DDNumericDataGroup extends DataDescriptor {
  ...
  public void getAsByteArray(byte[] buf, int bufOff, int bufLen) throws IOException {
    ...
    for (int i = 0; i < members.length; i++)
      if (members[i].tagID == HDFConstants.DFTAG_SD) {
        hdfFile.retrieveElementByID(members[i]).readData(buf, bufOff, bufLen);
        return;
      }
  }
