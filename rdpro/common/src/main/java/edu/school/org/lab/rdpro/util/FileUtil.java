...
//
  public static void concat(FileSystem fs, Path outFile, Path ... inFiles) throws IOException {
    ...
    try {
      //LOG.warn("Output file : "+outFile.toString());
      //LOG.warn("Input files : "+inFiles[0].toString());
      // todo: condition, outfile create empty file, close as well
      if (!fs.exists(outFile)){
        LOG.warn("Output file dose not exist, Create empty file on HDFS: "+outFile);
        FSDataOutputStream out = fs.create(outFile);
        out.close();
      }
      if(!inFiles[0].getParent().toString().equals(outFile.getParent().toString())) {
        Long startRename = System.nanoTime();
        // move input files to same directory
        Configuration conf = new Configuration();
        Vector<Path> inFilesVector = new Vector<>(inFiles.length);
        Vector<Path> newInFiles = new Vector<>(inFiles.length);
        for (int i = 0; i < inFiles.length; i++) {
          Path newPath = new Path(outFile.getParent().toString() + "/" + inFiles[i].getName());
          newInFiles.add(i, newPath);
          inFilesVector.add(i, inFiles[i]);
        }
        // rename in parallel
        try {
          Parallel.forEach(newInFiles.size(), (f1, f2) -> {
            for (int i = f1; i < f2; i++) {
              try {
                fs.rename(inFilesVector.get(i), newInFiles.get(i)); // check if go here
                LOG.info(" success rename parallel");
              } catch (IOException e) {
          ...
          throw new RuntimeException(String.format("Cannot parallel rename"), e);
        }
        //System.out.println(" move to parent dir: "+outFile.getParent().toString());
        //move(conf, inFiles[i], outFile.getParent()); // todo: change to copy file? otherwise exception when manually oconcat
        // call directly rename file
        //fs.rename(inFiles[i], newInFiles[i]); // check if go here
        //System.out.println("%%%%%%  ######## Successfully rename files on HDFS %%%%%%%% ######## seconds" + (System.nanoTime() - startRename) / 1E9);
        fs.concat(outFile, newInFiles.toArray(new Path[newInFiles.size()])); // todo: check if running // move infiles?
        LOG.info(" %%%%%%  ######## Successfully concat files on HDFS %%%%%%%% ######## seconds"+(System.nanoTime()-startRename)/1E9);
      }
      Long startConcat = System.nanoTime();
      fs.concat(outFile, inFiles); // todo: check if running // move infiles?
      // fs.concat(outFile, inFiles);
    } catch (Exception e) {
      LOG.warn(" %%%%%%  ######## EXCEPTION HDFS %%%%%%%% ########");
      LOG.warn(" %%%%%%  ######## %%%%%%%% ######## "+e.toString());
      LOG.warn(" %%%%%%  ######## Start to concat files manually %%%%%%%% ########");
      // Fall back to an inefficient concatenation by reading/writing.
      try (FSDataOutputStream out = fs.create(outFile)) {
        for (Path inFile : inFiles) {
          // inFile = new Path(outFile.getParent().toString()+"/"+inFile.getName());
