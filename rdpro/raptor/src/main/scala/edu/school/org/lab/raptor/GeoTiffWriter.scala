...
/**
 * A writer for GeoTiff files. It takes [[edu.school.org.lab.rdpro.geolite.RasterMetadata]] at construction
 * and a sequence of [[edu.school.org.lab.rdpro.geolite.ITile]]s to write.
...
class GeoTiffWriter[T](val outPath: Path, rasterMetadata: RasterMetadata, bare: Boolean = false,
  ...
  def write(tile: ITile[T]): Unit = {
    //    if (numBands!=0 && numBands!=tile.numComponents){
    //      logWarning(s"NumBands is : "+(numBands))
    //      logWarning(s"NumComponent is : "+(tile.numComponents))
    //      logWarning(s"NumBands is not match with tile of NumComponents")
    //      return
    //    }
    // numbands initially is 1?
    numBands = tile.numComponents // before: check numbands = 0, is initial, if numbands != numcomponent, warning, return
      ...
      if (j > tile.y2 || i > tile.x2 || tile.isEmpty(i, j)) {
        ...
        if (numBands == 1) {
          tile.getPixelValue(i, j) match {
            ...
            case ai: Array[Int] => for (b <- 0 until numBands)
              pixelOutput.write(ai(b), bitsPerSample(b))
            case af: Array[Float] => for (b <- 0 until numBands)
              pixelOutput.write(java.lang.Float.floatToIntBits(af(b)), bitsPerSample(b))
          }
        } else { // check tile . numcomponent = bands?
          tile.getPixelValue(i, j) match {
            case is: Array[Int] =>
              if (bitsPerSample.length < numBands) {
                logWarning(s"NumBands is : "+(numBands))
                logWarning(s"NumComponent is : "+(bitsPerSample.length))
                logWarning(s"NumBands is not match with bitsper sample of NumComponents")
                return
              }
              for (b <- 0 until numBands)
...
object GeoTiffWriter extends Logging {
  ...
  private def saveAsGeoTiffCompatibilityMode[T](rasterRDD: RDD[ITile[T]], outPath: String, opts: BeastOptions): Unit = {
    ...
    // todo: start time t0
    val startTimeT0: Long = System.nanoTime()
    val interimResults: Array[(RasterMetadata, String, String, Array[Int], Array[Int])] =
      sc.runJob(rasterRDD, (taskContext: TaskContext, tiles: Iterator[ITile[T]]) => {
        ...
        val taskTempDir: Path = new Path(outPath.toString + "/" + taskContext.taskAttemptId().toString)
        ...
        taskContext.addTaskFailureListener(deleteTempDir) // todo: loop over files and delete (tiledata, tileinfo)
        // todo: track writer, close writer before delete (loop over writer)
        fileSystem.deleteOnExit(taskTempDir) // todo: track writer, close writer before delete (loop over writer)
        ...
        for (tile <- tiles) {
          val writer = writers.getOrElseUpdate(tile.rasterMetadata, {
            //val filename = f"part-${taskContext.partitionId()}%05d-${writers.size}%03d_tiledata" // include task temp id taskAttemptId
            val filename = f"part-${taskContext.partitionId()}%05d-${writers.size}%03d_tiledata-${taskContext.taskAttemptId().toString}"
            //val geoTiffPath = new Path(outPath, filename)
            val geoTiffPath = new Path(outPath.toString + "/" + filename)
            println(" FILE PATH:" + geoTiffPath.toString)
        ...
        writers.map(w => {
          //val tileInfoPath: Path = new Path(outPath, f"part-${taskContext.partitionId()}%05d-${writers.size}%03d_tileinfo-${taskContext.taskAttemptId().toString}")
          val tileInfoPath: Path = new Path(outPath + "/" + f"part-${taskContext.partitionId()}%05d-${writers.size}%03d_tileinfo-${taskContext.taskAttemptId().toString}")
    ...
    //todo: second time t1
    val endTimeT1: Long = System.nanoTime()
    try {
      var totalTimePrepenhead: Long = 0
      var totalFileConcat: Int = 0
      var totalTimeGetInfo: Long = 0
    ...
    // todo: end time t2
    val endTimeT2: Long = System.nanoTime()
    val firstPart: Double = (endTimeT1 - startTimeT0) / 1E9
    val secondPart: Double = (endTimeT2 - endTimeT1) / 1E9
    logInfo(" Time to write first part (t2-t0): " + firstPart)
    logInfo(" Time to write second part (t1-t0): " + secondPart)
  }
  ...
  private def saveAsGeoTiffDistributedMode[T](rasterRDD: RDD[ITile[T]], outPath: String, opts: BeastOptions): Unit = {
    ...
    val interimFiles: Array[String] = sc.runJob(rasterRDD, (taskContext: TaskContext, tiles: Iterator[ITile[T]]) => {
      //val taskTempDir: Path = new Path(new Path(tempPath), taskContext.taskAttemptId().toString)
      val taskTempDir: Path = new Path(tempPath+"/"+taskContext.taskAttemptId().toString)
      val fileSystem: FileSystem = taskTempDir.getFileSystem(opts.loadIntoHadoopConf())
      fileSystem.mkdirs(taskTempDir)
      // Cleanup handler
      val deleteTempDir: (TaskContext, Throwable) => Unit = (_, _) => fileSystem.delete(taskTempDir, true)
      ...
      for (tile <- tiles) {
        val writer = writers.getOrElseUpdate(tile.rasterMetadata, {
          //val filename = f"part-${taskContext.partitionId()}%05d-${writers.size}%03d_tiledata-${taskContext.taskAttemptId().toString}"
          val filename = f"part-${taskContext.partitionId()}%05d-${writers.size}%03d_tiledata-${taskContext.taskAttemptId().toString}.tif"
          //val geoTiffPath = new Path(taskTempDir, filename)
          val geoTiffPath = new Path(taskTempDir.toString+"/"+filename)
          logInfo(" FILE PATH:" + geoTiffPath.toString)
  ...
  private def saveAsGeoTiffDistributedModeColRow[T](rasterMetadata: RasterMetadata,rasterRDD: RDD[ITile[T]], outPath: String, opts: BeastOptions): Unit = {
    val sc: SparkContext = rasterRDD.sparkContext
    val tempPath: String = new File(outPath, "temp").getPath
    val interimFiles: Array[String] = sc.runJob(rasterRDD, (taskContext: TaskContext, tiles: Iterator[ITile[T]]) => {
      val taskTempDir: Path = new Path(new Path(tempPath), taskContext.taskAttemptId().toString)
      ...
      for (tile <- tiles) {
        val writer = writers.getOrElseUpdate(tile.rasterMetadata, {
          ...
          tile.rasterMetadata.gridToModel(0, 0, pt)
          val xp1 = pt.x
          val yp1 = pt.y
          rasterMetadata.modelToGrid(xp1, yp1, pt)
          val filename = f"part-${taskContext.partitionId()}%05d-${writers.size}%03d-${pt.x.toInt}-${pt.y.toInt}.tif"
          val geoTiffPath = new Path(taskTempDir, filename)
          new GeoTiffWriter[T](geoTiffPath, tile.rasterMetadata, false, opts)
        })
        writer.write(tile)
  ...
  def saveAsGeoTiffColRow[T](rasterMetadata: RasterMetadata,rasterRDD: RDD[ITile[T]], outPath: String, opts: BeastOptions): Unit = {
    def maxArrayB(a: Array[Byte], b: Array[Byte]): Array[Byte] = {
      for (i <- a.indices)
        a(i) = a(i) max b(i)
      a
    ...
    opts.getString(WriteMode, "distributed") match {
      case "distributed" => saveAsGeoTiffDistributedMode(rasterRDD, outPath, opts)
      case "compatibility" => saveAsGeoTiffCompatibilityMode(rasterRDD, outPath, opts)
      case "distributedcolrow" => saveAsGeoTiffDistributedModeColRow(rasterMetadata,rasterRDD, outPath, opts)
    }
  }
  /**
  ...
  def prependGeoTiffHeader(rasterMetadata: RasterMetadata, bitsPerSample: Array[Int],
    ...
    val software: String = "RDPRO HEAD"
    ...
    val concatStart: Long = System.nanoTime()
    // Concatenate both files
    // todo: add timmer check
    FileUtil.concat(fileSystem, outPath, (headerPath +: tileDataPath): _*) //todo: check concat most time
    //val concatEnd: Long = System.nanoTime()
    //println(" %%%%%%%% ####### Concat time: "+ (concatEnd-concatStart)/1E9)
    //logInfo(" %%%%%%%% ####### Concat time: "+ (concatEnd-concatStart)/1E9)
  }
