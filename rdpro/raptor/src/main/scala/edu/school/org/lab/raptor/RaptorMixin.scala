...
import edu.school.org.lab.rdpro.cg.SpatialDataTypes.{RasterRDD, SpatialRDD}
import edu.school.org.lab.rdpro.common.BeastOptions
import edu.school.org.lab.rdpro.geolite.{ITile, RasterMetadata}
import RasterOperationsFocal.InterpolationMethod
...
trait RaptorMixin {
  ...
  implicit class RaptorMixinOperations3[T](raster: RasterRDD[T]) {
     ...
     * @return a new raster RDD with the desired coordinate reference system.
     */
    def reproject(targetCRS: CoordinateReferenceSystem, interpolationMethod: InterpolationMethod.InterpolationMethod =
        InterpolationMethod.NearestNeighbor)(implicit t: ClassTag[T]): RasterRDD[T] =
      RasterOperationsFocal.reproject(raster, targetCRS, interpolationMethod)
     ...
     */
    def rescale(rasterWidth: Int, rasterHeight: Int, interpolationMethod: InterpolationMethod.InterpolationMethod =
        InterpolationMethod.NearestNeighbor)(implicit t: ClassTag[T]): RasterRDD[T] =
      RasterOperationsFocal.rescale(raster, rasterWidth, rasterHeight, interpolationMethod)
