...
class MemoryTile[T](tileID: Int, metadata: RasterMetadata, var _rasterFeature: RasterFeature = null)
                   (implicit @transient t: ClassTag[T]) extends ITile[T](tileID, metadata, _rasterFeature) {

  if (_rasterFeature == null)
    _rasterFeature = RasterFeature.createEmpty()
   ...
   * @return
   */
  private def makeSchema(names: Array[String], types: Array[DataType]): StructType = {
    val numAttributes: Int = if (names != null) names.length
    else if (types != null) types.length
    else 0
    val fields = new Array[StructField](numAttributes)
    for (i <- 0 until numAttributes) {
      ...
      fields(i) = StructField(name, fieldType)
    }
    StructType(fields)
  }
