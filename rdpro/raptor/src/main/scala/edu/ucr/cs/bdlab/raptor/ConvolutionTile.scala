...
import org.apache.spark.sql.types.{ArrayType, DataType, FloatType, StructType}
...
abstract class AbstractConvolutionTile[T](tileID: Int, metadata: RasterMetadata, rasterFeature: RasterFeature,
                                          val w: Int, val weights: Array[Float],
                                          val sourceTileID: Int,
                                          private[raptor] var pixelValues: Array[Float] = null,
                                          private[raptor] var pixelExists: BitArray = null)
  extends ITile[T](tileID, metadata, rasterFeature) {
  ...
  def merge(other: AbstractConvolutionTile[T]): AbstractConvolutionTile[T] = {
    if (this.tileID != sourceTileID) {
      ...
      val fullTile: AbstractConvolutionTile[T] = if (numComponents == 1)
        new ConvolutionTileSingleBand(this.tileID, this.metadata, this.rasterFeature, this.w, this.weights, this.tileID).asInstanceOf[AbstractConvolutionTile[T]]
      else
        new ConvolutionTileMultiBand(this.tileID, this.metadata, this.rasterFeature, this.numComponents, this.w, this.weights, this.tileID).asInstanceOf[AbstractConvolutionTile[T]]
      fullTile.merge(this)
...
@DefaultSerializer(classOf[ConvolutionTileSerializer])
class ConvolutionTileSingleBand(tileID: Int, metadata: RasterMetadata, rasterFeature: RasterFeature, w: Int, weights: Array[Float],
                                stid: Int,
                               @transient val values: Array[Float] = null, @transient val exists: BitArray = null)
  extends AbstractConvolutionTile[Float](tileID, metadata, rasterFeature, w, weights, stid, values, exists) {
...
@DefaultSerializer(classOf[ConvolutionTileSerializer])
class ConvolutionTileMultiBand(tileID: Int, metadata: RasterMetadata, rasterFeature: RasterFeature,
  ...
  extends AbstractConvolutionTile[Array[Float]](tileID, metadata, rasterFeature, w, weights, stid, values, exists) {
