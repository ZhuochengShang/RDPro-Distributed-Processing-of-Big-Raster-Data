...
object RasterOperationsFocal {
   ...
   * @param targetMetadata the desired metadata
   * @return the new raster with the target metadata
   */
  def reshapeAverage[T: ClassTag](raster: RasterRDD[T], targetMetadata: RasterMetadata): RasterRDD[T] = {
    ...
    // Adjust number of partitions to have roughly 512 MB per partition assuming four bytes per pixel
    // Note that we use 512 MB rather than the default HDFS block size (128 MB) because tiles are usually compressed
    val numTargetPartitions = (4.0 * targetMetadata.rasterWidth * targetMetadata.rasterHeight / 512 / 1024 / 1024).toInt max raster.sparkContext.defaultParallelism
   ...
   * @return the new raster with the target metadata
   */
  def reshapeNN[T: ClassTag](raster: RasterRDD[T], targetMetadata: RasterMetadata): RasterRDD[T] = {
    ...
    val intermediateTiles: RDD[(Int, MemoryTile[T])] = raster.flatMap(sourceTile => {
      ...
      // 3- Use target metadata model-to-grid to convert from target model space to target grid space
      // To do all these steps efficiently, we concatenate all these transformations into one
      var (sourceToTarget, targetToSource) = cache.getOrElseUpdate(sourceTile.rasterMetadata.srid, {
        val sourceG2M = new AffineTransform2D(sourceTile.rasterMetadata.g2m)
        ...
        val targetM2G = new AffineTransform2D(targetMetadata.g2m).inverse()
        val sourceToTarget: MathTransform = concatenateTransforms(sourceG2M, transformationInfo.sourceSnap, transformationInfo.mathTransform, targetM2G)
    ...
    // Adjust number of output partitions to have roughly 512 MB per partition assuming four bytes per pixel
    // Note that we use 512 MB rather than the default HDFS block size (128 MB) because tiles are usually compressed
    val numTargetPartitions: Int = (4.0 * targetMetadata.rasterWidth * targetMetadata.rasterHeight / 512 / 1024 / 1024).toInt max raster.sparkContext.defaultParallelism
    val partitioner = new HashPartitioner(numTargetPartitions)
    intermediateTiles.reduceByKey(partitioner, (t1, t2) => {
   ...
   * @return
   */
  def reproject[T: ClassTag](raster: RasterRDD[T], targetCRS: CoordinateReferenceSystem,
                             interpolationMethod: InterpolationMethod.InterpolationMethod =
                             InterpolationMethod.NearestNeighbor): RasterRDD[T] = interpolationMethod match {
    case InterpolationMethod.Average => reshapeAverage(raster, raster.first.rasterMetadata.reproject(targetCRS))
    case InterpolationMethod.NearestNeighbor => reshapeNN(raster, raster.first.rasterMetadata.reproject(targetCRS))
  }
   ...
   */
  def rescale[T: ClassTag](raster:RasterRDD[T], rasterWidth: Int, rasterHeight: Int,
                           interpolationMethod: InterpolationMethod.InterpolationMethod =
                           InterpolationMethod.NearestNeighbor): RasterRDD[T] = interpolationMethod match {
    case InterpolationMethod.Average => reshapeAverage(raster, raster.first.rasterMetadata.rescale (rasterWidth, rasterHeight) )
    case InterpolationMethod.NearestNeighbor => reshapeNN(raster, raster.first.rasterMetadata.rescale (rasterWidth, rasterHeight) )
  }
  ...
  def retile[T: ClassTag](raster: RasterRDD[T], tileWidth: Int, tileHeight: Int): RasterRDD[T] = {
    val sourceMetadata = raster.first().rasterMetadata
    val targetMetadata = sourceMetadata.retile(tileWidth, tileHeight)
    // Notice that for retile, there is a one-to-one correspondence in pixels so we use the more efficient NN method
    reshapeNN(raster, targetMetadata)
  }
  ...
  def slidingWindowImp[T: ClassTag, U: ClassTag](raster: RasterRDD[T], w: Int, f: (Array[T], Array[Boolean]) => U):
  RasterRDD[U] = {
    import RaptorMixin._
    assert(raster.allMetadata.length == 1, "Raster input to window calculation function cannot have mixed metadata")
    // calculate w_x, w_y
    // each tile ID check how man tile valid in current boundary
    val intermediateTiles: RDD[(Int, WindowOfTiles[T,U])] = raster.flatMap(sourceTile => {
      val metadata = sourceTile.rasterMetadata
      val w_x = math.ceil(w / metadata.tileWidth.toFloat)
      val w_y = math.ceil(w / metadata.tileHeight.toFloat)
      val numTiles = ((2 * w_x + 1) *  (2 * w_y + 1)).toInt
      // Compute the window of pixels that will be affected by current tile *inclusive*
      val windowX1 = (sourceTile.x1 - w) max metadata.x1
      val windowY1 = (sourceTile.y1 - w) max metadata.y1
      val windowX2 = (sourceTile.x2 + w) min (metadata.x2 - 1)
      ...
      val tileRow2 = (windowY2 - metadata.y1) / metadata.tileHeight
      // Create an array of all target tile IDs
      val targetTileIDs = new Array[(Int,Int)]((tileCol2 - tileCol1 + 1) * (tileRow2 - tileRow1 + 1))
      var i = 0
      for (tileRow <- tileRow1 to tileRow2; tileCol <- tileCol1 to tileCol2) {
        var targetTileIDs_numTiles = 0
        // keep only valid one
        if (tileRow == 0 || tileRow == metadata.numTilesY - 1) {
          if (tileCol == 0 || tileCol == metadata.numTilesX - 1) {
            targetTileIDs_numTiles = 4
          } else {
            targetTileIDs_numTiles = 6
          }
        } else if (tileCol == 0 || tileCol == metadata.numTilesX - 1) {
          targetTileIDs_numTiles = 6
        } else targetTileIDs_numTiles = numTiles
        targetTileIDs(i) = ( tileRow * metadata.numTilesX + tileCol, targetTileIDs_numTiles)
        i += 1
      }
      // Now, compute the target tiles and return them
      targetTileIDs.iterator.map(targetTileID => {
        val numTiles = targetTileID._2
        val targetTile: WindowOfTiles[T,U] = WindowOfTiles.create[T,U](targetTileID._1, metadata, targetTileID._2, w, f) // pass user defined function

        //targetTile.addTile(sourceTile.tileID) // add current source tile
        targetTile.addTile(sourceTile)

        (targetTileID._1, targetTile)
      })
    })

    // Merge all values from intermediate tiles
    val finalWindowTiles: RDD[(Int, WindowOfTiles[T,U])] = intermediateTiles.reduceByKey((t1, t2) => {
    // assert t1, t2 both are not final
      assert(t1.finalValues == null && t2.finalValues == null)
      t1.merge(t2)
      t1
    })

    finalWindowTiles.map( windowTile => {
      val finalTile: ITile[U] = windowTile._2.computeFinaleValue()
//      val finalTile = new MemoryTile[U](windowTile._2.tileID,windowTile._2.metadata)
//
//      for (y <- finalTile.y1 to finalTile.y2; x <- finalTile.x1 to finalTile.x2) {
//        //val offset = math.min(y * (finalTile.x2 - finalTile.x1) + x,finalValues.length-1)
//        val offset = windowTile._2.metadata.rasterWidth * (y - windowTile._2.metadata.y1) + x
//        //val offset = x * (finalTile.x2 - finalTile.x1) + y
//        finalTile.setPixelValue(x,y, finalValues(offset))
//      }
      finalTile
    })

    // use final tile to create memory tile
  }
  ...
  private val averageComputer: Map[DataType, Array[Float] => _] = Map(
    ...
    (IntegerType, (acc: Array[Float]) => (acc(0) / acc(1)).round.toInt),
    (LongType, (acc: Array[Float]) => (acc(0).toDouble / acc(1)).round.toLong),
    ...
    (new ArrayType(IntegerType, false), (acc: Array[Float]) => {
      ...
      for (i <- result.indices)
        result(i) = (acc(i) / acc(result.length)).round.toInt
      result
    }),
    (new ArrayType(LongType, false), (acc: Array[Float]) => {
      val result = new Array[Long](acc.length - 1)
      for (i <- result.indices)
        result(i) = (acc(i).toDouble / acc(result.length)).round.toLong
      result
    }),
