...
object RasterOperationsLocal {
  ...
  def filterPixelWithoutWrapper[T: ClassTag](inputRaster: RasterRDD[T], filter: T => Boolean): RasterRDD[T] =
    inputRaster.map(inputTile => {
      val res = new MemoryTile[T](inputTile.tileID, inputTile.rasterMetadata, inputTile.rasterFeature)
      for (j <- inputTile.y1 to inputTile.y2; i <- inputTile.x1 to inputTile.x2) {
        if(inputTile.isEmpty(i, j) || !filter(inputTile.getPixelValue(i, j))) {
        }else {
          res.setPixelValue(i,j,inputTile.getPixelValue(i,j))
        }
      }
      res
    })

  def MapPixelWithoutWrapper(inputRaster: RasterRDD[Array[Int]]): RasterRDD[Int]  =
    inputRaster.map(inputTile => {
      val res = new MemoryTile[Int](inputTile.tileID, inputTile.rasterMetadata, inputTile.rasterFeature)
      for (j <- inputTile.y1 to inputTile.y2; i <- inputTile.x1 to inputTile.x2) {
        if(inputTile.isDefined(i,j)){
          val pixelValue = inputTile.getPixelValue(i,j)
          val newPixelValue = pixelValue(0)+pixelValue(1)
          res.setPixelValue(i,j,newPixelValue)
        }
      }
      res
    })
