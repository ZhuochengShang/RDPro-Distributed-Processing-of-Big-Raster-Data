 ...
 */
class CompactIntersectionsTileBreakerByKey(intersections: Iterator[((Timestamp,Int), Intersections)])
  extends Iterator[((Timestamp,Long), CompactIntersections)] {
  ...
  override def next(): ((Timestamp,Long), CompactIntersections) = {
    val firstRange: Int = i
    val tileID = currentIntersections.getTileID(i)
   // var timestamp = compactIntersections.getTimeStamp(i)
    do {
      i += 1
    ...
    ((timestamp,metadataTileID), compactIntersections)
  }
}
