...
class SlidingWindowTile[T, U](val tileID: Int, metadata: RasterMetadata, rasterFeature: RasterFeature, w: Int, winFunc: (Array[T], Array[Boolean]) => U)
  ...
  private def computeResultTile(): Unit = {
    if (_resultTile != null)
      return
    val tile = new MemoryTile[U](tileID, metadata, rasterFeature)
