...
import org.apache.spark.sql.types.{ByteType, DataType, DoubleType, FloatType, IntegerType, LongType, ShortType, StructType}

import scala.annotation.tailrec
import scala.reflect.ClassTag

@DefaultSerializer(classOf[ITileSerializer[Any]])
class MemoryTile2[T](override val tileID: Int, override val rasterMetadata: RasterMetadata, override val rasterFeature: RasterFeature)
                    (implicit @transient t: ClassTag[T]) extends ITile[T](tileID, rasterMetadata, rasterFeature) {
