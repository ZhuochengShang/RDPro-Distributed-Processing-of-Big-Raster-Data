...
class ConvolutionTileSerializer extends Serializer[AbstractConvolutionTile[Any]] {
  ...
  override def read(kryo: Kryo, input: Input, klass: Class[AbstractConvolutionTile[Any]]): AbstractConvolutionTile[Any] = {
    ...
    val t = if (numComponents == 1)
      new ConvolutionTileSingleBand(tileID, metadata, rasterFeature, w, weights, sourceTileID, pixelValues, pixelExists)
    else
      new ConvolutionTileMultiBand(tileID, metadata, rasterFeature, numComponents, w, weights, sourceTileID, pixelValues, pixelExists)
