...
class GeoTiffWriterTest extends AnyFunSuite with ScalaSparkTest {
  ...
  test(" test with write row tiled file"){
    val crs = CRS.decode("EPSG:32610")
    val srid = CRSServer.crsToSRID(crs)
    val metadata = new RasterMetadata(0, 0, 10980, 10980, 10980, 1, srid,
      new AffineTransform())
    val originalTile1 = new MemoryTile[Int](9375, metadata, RasterFeature.create(Array("fileName"), Array("testFile.tif")))
    originalTile1.setPixelValue(10978, 9375, 121)
    originalTile1.setPixelValue(10979, 9375, 121)

    //val geotTiffPath = new Path(scratchPath, "temp.tif")
    val geotTiffPath = new Path("/Users/clockorangezoe/Downloads/waterfarm.tif/testrow.tif")
    val fileSystem = geotTiffPath.getFileSystem(sparkContext.hadoopConfiguration)
    ...
    geoTiffReader.initialize(fileSystem, geotTiffPath.toString, "0", new BeastOptions)
    assertResult(metadata.srid)(geoTiffReader.metadata.srid)
    val actualTile1 = geoTiffReader.readTile(9375)
    println(originalTile1.getPixelValue(10978, 9375))
    print(actualTile1.getPixelValue(10978, 9375))
    assertResult(originalTile1.getPixelValue(10978, 9375))(actualTile1.getPixelValue(10978, 9375))
    assertResult(originalTile1.getPixelValue(10979, 9375))(actualTile1.getPixelValue(10979, 9375))
    geoTiffReader.close()
  }
