...
class RasterOperationsFocalTest extends AnyFunSuite with ScalaSparkTest {
  test("retile with the reshape method") {
    ...
    val sourceMetadata = new RasterMetadata(0, 0, 100, 100, 100, 100, 4326,
      new AffineTransform())
    val inputTile: MemoryTile[Short] = new MemoryTile(0, sourceMetadata, RasterFeature.create(Array("fileName"),Array("testFile.tif")))
  ...
  test("resample merge intermediate tiles") {
    ...
    val raster = RasterOperationsGlobal.rasterizePixels(pixels, metadata, RasterFeature.create(Array("fileName"),Array("testFile.tif")))
  ...
  test("reshape average with a source tile not covering the center of a target pixel") {
    ...
    val sourceRaster: RasterRDD[Int] = RasterOperationsGlobal.rasterizePixels(pixels, sourceMetadata, RasterFeature.create(Array("fileName"),Array("testFile.tif")))
...
//    ))
//    val raster: RasterRDD[Int] = RasterOperationsGlobal.rasterizePixels(pixels, metadata, RasterFeature.create(Array("fileName"), Array("testFile.tif")))
//    val rasterDate: RDD[(Timestamp, ITile[Int])] = raster.map(t => (Timestamp.valueOf("2019-02-23 05:00:00"),t))
//    val reshaped = RasterOperationsFocal.reshapeNNwithNullKeys(rasterDate,Timestamp.valueOf("2019-02-23 05:00:00"),
//      _ => RasterMetadata.create(90, 0, 100, 10, 4326, 10, 10, 10, 10))
//    assertResult(1)(reshaped.count())
//  }
