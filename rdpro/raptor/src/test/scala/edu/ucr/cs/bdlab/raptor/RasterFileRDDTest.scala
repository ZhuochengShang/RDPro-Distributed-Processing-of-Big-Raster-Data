...
import org.locationtech.jts.geom.{CoordinateSequenceFactory, Envelope, GeometryFactory, PrecisionModel}
...
class RasterFileRDDTest extends AnyFunSuite with ScalaSparkTest {
  ...
  test("Override CRS"){
    val file = locateResource("/rasters/FRClouds.tif")
  ...
  test("Build and use index") {
    ...
    assertResult(1)(matchingFiles.length)
    val matchingFiles2 = RasterFileRDD.selectFiles(FileSystem.get(sparkContext.hadoopConfiguration), dir.toString,
      geometryFactory.toGeometry(new Envelope(-100, -99, -61, -60)))
    assertResult(0)(matchingFiles2.length)
  }

  test("Read RDD as Row ITile") {
    val file = locateResource("/rasters/glc2000_small.tif")
    val fileTileRDD = new RasterFileRDD(sparkContext, file.toString, SpatialFileRDD.MaxSplitSize -> 4096)
    assertResult(2)(fileTileRDD.getNumPartitions)
    assertResult(8)(fileTileRDD.count)
