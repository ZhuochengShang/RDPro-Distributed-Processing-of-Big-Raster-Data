...
class SlidingWindowTileTest extends AnyFunSuite with ScalaSparkTest {
  test("corner tile") {
    val metadata = new RasterMetadata(0, 0, 100, 100, 10, 10, 4326,
      new AffineTransform())
    val windowTile = new SlidingWindowTile(0, metadata, RasterFeature.create(Array("fileName"),Array("testFile.tif")), 1, (values: Array[Int], defined: Array[Boolean]) => {
          // Sum all the defined values
          values.zip(defined).filter(_._2).map(_._1).sum
        })
    val tile1 = new MemoryTile[Int](0, metadata,  RasterFeature.create(Array("fileName"),Array("testFile.tif")))
    ...
    val tile2 = new MemoryTile[Int](1, metadata,RasterFeature.create(Array("fileName"),Array("testFile.tif")))
    tile2.setPixelValue(10, 9, 3)
    windowTile.addTile(tile2)
    val tile3 = new MemoryTile[Int](10, metadata, RasterFeature.create(Array("fileName"),Array("testFile.tif")))
    tile3.setPixelValue(9, 10, 15)
    windowTile.addTile(tile3)
    val tile4 = new MemoryTile[Int](11, metadata, RasterFeature.create(Array("fileName"),Array("testFile.tif")))
  ...
  test("corner tile with merge") {
    ...
    val windowTile2 = new SlidingWindowTile(0, metadata, RasterFeature.create(Array("fileName"),Array("testFile.tif")), 1, (values: Array[Int], defined: Array[Boolean]) => {
          // Sum all the defined values
          values.zip(defined).filter(_._2).map(_._1).sum
        })
