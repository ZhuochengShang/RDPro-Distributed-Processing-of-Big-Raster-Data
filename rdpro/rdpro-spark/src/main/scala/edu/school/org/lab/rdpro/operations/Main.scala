...
import edu.school.org.lab.rdpro.util.OperationHelper.{ParsedCommandLineOptions, printOperationUsage}
...
object Main extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    conf.setAppName("rdpro/" + parsedCLO.operation.metadata.shortName)
    ...
    try {
      ...
      if (conf.contains(CRSServer.CRSServerPort))
        parsedCLO.options.set(CRSServer.CRSServerPort, conf.get(CRSServer.CRSServerPort))
