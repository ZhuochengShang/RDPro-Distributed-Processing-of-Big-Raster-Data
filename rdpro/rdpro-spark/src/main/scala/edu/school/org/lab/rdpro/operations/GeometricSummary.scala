...
import edu.school.org.lab.rdpro.synopses.{BoxCounting, Summary, SummaryAccumulator}
...
object GeometricSummary extends CLIOperation with Logging {
  ...
  def computeForFeaturesWithOutputSize(features : SpatialRDD, opts : BeastOptions) : Summary =
    edu.school.org.lab.rdpro.synopses.Summary.computeForFeatures(features, new FeatureWriterSize(opts))
