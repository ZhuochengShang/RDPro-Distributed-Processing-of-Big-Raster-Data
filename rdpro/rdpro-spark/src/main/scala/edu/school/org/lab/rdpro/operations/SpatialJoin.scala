...
import edu.school.org.lab.rdpro.cg.{GeometryQuadSplitter, PlaneSweepSpatialJoinIterator, SpatialPartitioner}
...
import edu.school.org.lab.rdpro.indexing.{CellPartitioner, IndexHelper}
import edu.school.org.lab.rdpro.io.{SpatialFileRDD, SpatialOutputFormat, SpatialWriter}
import edu.school.org.lab.rdpro.synopses.{Summary, Synopsis, UniformHistogram}
import edu.school.org.lab.rdpro.util.{MathUtil, OperationMetadata, OperationParam}
...
""",
  ...
  def spatialJoin(r1: SpatialRDD, r2: SpatialRDD, joinPredicate: ESJPredicate = ESJPredicate.Intersects,
    ...
    if (quadSplitThreshold > 0) {
      ...
      val datasets: Array[(SpatialRDD, Boolean)] = Array((r1, removeGeometry1), (r2, removeGeometry2)).map(rr => Future {
        var features = rr._1
        val summary = if (joinMethod == ESJDistributedAlgorithm.PBSM || joinMethod == ESJDistributedAlgorithm.SJMR) {
   ...
   */
  private[rdpro] def spatialJoinIntersectsPlaneSweepFeatures[T1 <: IFeature, T2 <: IFeature]
