...
public class MultilevelPlotTest extends JavaSparkTest {
  ...
  public void testCreateCompactImageTiles() throws IOException {
    File inputFile = new File(scratchPath().toString(), "in.point");
    File outputFile = new File(scratchPath().toString(), "out");
    copyResource("/viztest.point", inputFile);
    ...
    BeastOptions opts = new BeastOptions(false)
        ...
        .set("threshold", 0)
        .set("vflip", false)
        .set(MultilevelPlot.ConcatenateImageTiles(), true)
        .set(SpatialWriter.OutputFormat(), "point");
    MultilevelPlot.run(opts, new String[] {inputFile.getPath()}, new String[] {outputFile.getPath()}, sparkContext());

    String[] expectedFiles = {
        MultilevelPlot.MasterTileFileName(),
        MultilevelPlot.TilesFileName()
    };
    for (String expectedFile : expectedFiles)
      assertTrue(String.format("Expected tile '%s' not found", expectedFile),
          new File(outputFile, expectedFile).exists());

    // Make sure that all temporary directories were cleaned up
    for (File file : Objects.requireNonNull(outputFile.listFiles())) {
      assertFalse("No subdirectories should exist", file.isDirectory());
    }
  }

  public void testCreateImageTilesWithFlatAndPyramid() throws IOException {
    File inputFile = new File(scratchPath().toString(), "in.point");
    File outputFile = new File(scratchPath().toString(), "out");
    copyResource("/viztest.point", inputFile);
    ...
    BeastOptions opts = new BeastOptions(false)
        ...
        .set("levels", 3)
        .set("threshold", 0)
        .set(MultilevelPlot.DeepestFlatLevel(), 1)
        .set("vflip", false);
  ...
  public void testCreateImageAndDataTiles() throws IOException {
    ...
    BeastOptions opts = new BeastOptions(false)
        ...
        .set("levels", 3)
        .set("vflip", false)
        .set(MultilevelPlot.ImageTileThreshold(), 8 * 2 + GeometryHelper.FixedGeometryOverhead)
        .set(SpatialWriter.OutputFormat(), "point")
        .set(GeometricPlotter.PointSize, 0);
    MultilevelPlot.run(opts, new String[] {inputFile.getPath()}, new String[] {outputFile.getPath()}, sparkContext());
  ...
  public void testCreateDataTilesWithDifferentInputOutputFormats() throws IOException {
    ...
    // Set the threshold equal to one point
        .set(MultilevelPlot.ImageTileThreshold(), 8 * 2 + GeometryHelper.FixedGeometryOverhead)
        .set(SpatialWriter.OutputFormat(), "wkt(0)")
        .set(GeometricPlotter.PointSize, 0);
    MultilevelPlot.run(opts, new String[] {inputFile.getPath()}, new String[] {outputFile.getPath()}, sparkContext());
  ...
  public void testSkipDataTilesAndReuseIndexWithPlotterClass() throws IOException {
    ...
    opts.set(MultilevelPlot.IncludeDataTiles(), false)
        .set(MultilevelPlot.ImageTileThreshold(), 8 * 2 + GeometryHelper.FixedGeometryOverhead)
        .set(GeometricPlotter.PointSize, 0)
        .set(CommonVisualizationHelper.VerticalFlip, false);
