...
public class MultilevelPyramidPlotHelperTest extends JavaSparkTest {
  ...
  public void testClassifyTileWithBuffer() {
    EnvelopeNDLite mbr = new EnvelopeNDLite(2, 0.0, 0.0, 1024.0, 1024.0);
    UniformHistogram h = new UniformHistogram(mbr, 1024, 1024);
    h.addEntry(new int[] {252, 252}, 100000);
    // Set buffer size to approximately five pixels assuming a 256x256 tile
    double buffer = 5 / 256.0;
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ImageTile,
        MultilevelPyramidPlotHelper.classifyTile(h, 1000, buffer, 2, 1, 1));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ImageTile,
        MultilevelPyramidPlotHelper.classifyTile(h, 1000, buffer, 10, 251, 251));
  }
