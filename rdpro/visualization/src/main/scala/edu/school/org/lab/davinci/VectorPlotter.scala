...
class VectorPlotter extends Plotter {
  ...
  override def setup(opts: BeastOptions): Unit = {
    super.setup(opts)
    this.pointSize = opts.getFloat(VectorPlotter.PointSize, 7.0f)
    this.fillColor = CommonVisualizationHelper.getColor(opts.getString(VectorPlotter.FillColor, "#8888"))
    this.strokeColor = CommonVisualizationHelper.getColor(opts.getString(VectorPlotter.StrokeColor, "black"))
    this.strokeWidth = opts.getFloat(VectorPlotter.StrokeWidth, 1.0f)
    this.svgTitle = opts.getString(VectorPlotter.Title)
  }
  ...
  override def writeImage(layer: Canvas, out: OutputStream, vflip: Boolean): Unit =
    layer.asInstanceOf[VectorCanvas].writeAsSVG(out, pointSize / 2.0f, strokeWidth, strokeColor, fillColor, vflip)
  ...
  private[davinci] def getTitle(feature: IFeature): String = {
    var title = VectorPlotter.AttributeNameRegexp.replaceAllIn(svgTitle, m => feature.getAs(m.group(1)).toString)
    title = VectorPlotter.AttributeNumberRegexp.replaceAllIn(title, m => feature.get(m.group(1).toInt + 1).toString)
    title
  }
}

object VectorPlotter {
  @OperationParam(
