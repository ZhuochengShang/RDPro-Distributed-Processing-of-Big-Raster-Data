...
import edu.school.org.lab.rdpro.geolite.{EnvelopeNDLite, GeometryHelper, IFeature}
import edu.school.org.lab.rdpro.io.{FeatureWriter, SpatialWriter}
import edu.school.org.lab.rdpro.synopses.{AbstractHistogram, UniformHistogram}
import edu.school.org.lab.rdpro.util.{OperationHelper, OperationParam}
import edu.school.org.lab.rdpro.cg.SpatialDataTypes.{JavaPartitionedSpatialRDD, JavaSpatialRDD, PartitionedSpatialRDD, SpatialRDD}
import edu.school.org.lab.rdpro.synopses._
import edu.school.org.lab.rdpro.util.{IntArray, OperationHelper, OperationParam}
...
- Count(c): Create n partitions such that each partition contains around c records""",
   ...
   */
  private[rdpro] def summarizeDataset(features: SpatialRDD, partitionerClass: Class[_ <: SpatialPartitioner],
   ...
   */
  private[rdpro] def runDuplicateAvoidance(features: SpatialRDD): SpatialRDD = {
  ...
  lazy val partitioners: Map[String, Class[_ <: SpatialPartitioner]] = {
    ...
    val partitionerClasses: java.util.List[String] = OperationHelper.readConfigurationXML("rdpro.xml").get("SpatialPartitioners")
