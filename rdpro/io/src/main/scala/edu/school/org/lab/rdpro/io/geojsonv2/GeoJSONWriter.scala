 ...
 */
package edu.school.org.lab.rdpro.io.geojsonv2

import com.fasterxml.jackson.core.util.DefaultPrettyPrinter
import com.fasterxml.jackson.core.{JsonFactory, JsonGenerator}
import edu.school.org.lab.rdpro.geolite.{EnvelopeND, PointND}
...
class GeoJSONWriter(out: OutputStream, schema: StructType, jsonLineFormat: Boolean = false, prettyPrint: Boolean = true,
  ...
  val iID: Int = schema.indexWhere(f => f.name == "id" || f.name == "_id")

  /** The index of all remaining attributes that go into the "properties" field */
  val iProperties: Array[Int] = (0 until schema.length).toArray.filter(i => i != iGeom && i != iID)
      ...
      case "MultiPolygon" =>
        ...
        for (iPoly <- 0 until multiPolygon.getNumGeometries) {
          ...
          for ($iRing <- 0 until subpoly.getNumInteriorRing + 1) {
            ...
            else subpoly.getInteriorRingN($iRing - 1)).asInstanceOf[LinearRing]
