...
object Cat extends CLIOperation with Logging {
  ...
  @OperationParam(description = "SRID of the output", defaultValue = "Same as input")
  val OutSRID = "osrid"
  ...
  override def run(opts: BeastOptions, inputs: Array[String], outputs: Array[String], sc: SparkContext): Any = {
    ...
    val osrid = opts.get(OutSRID)
    if (osrid.isDefined)
      features = Reprojector.reprojectRDD(features, osrid.get.toInt)
    // Store to the output
