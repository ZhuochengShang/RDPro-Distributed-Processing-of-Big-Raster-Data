...
public class RTreeFeatureWriterTest extends JavaSpatialSparkTest {
  ...
  public void testWriteTimestamp() throws IOException {
    ...
    writer.initialize(rtreePath, conf);
    IFeature[] features = {
        new Feature(new Object[] {new PointND(GeometryReader.DefaultGeometryFactory, 1.0, 2.4), "abc", new java.sql.Timestamp(10000000L)}, null),
    };
    for (IFeature f : features) {
      writer.write(f);
    }
    ...
    RTreeGuttman rtree = new RTreeGuttman(4, 8);
    rtree.readFields(in, rtreeSize, input -> null);
    Iterable<RTreeGuttman.Entry> results = rtree.search(0, 0, 4, 4);
    int resultCount = 0;
    for (Object o : results) {
      resultCount++;
    }
