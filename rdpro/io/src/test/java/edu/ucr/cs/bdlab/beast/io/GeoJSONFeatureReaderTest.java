...
public class GeoJSONFeatureReaderTest extends JavaSpatialSparkTest {
  public void testReadPoints() throws IOException {
    Path inFile = new Path(locateResource("/point.geojson").getPath());
  ...
  public void testReadPointsWithIntegerCoordinates() throws IOException {
    Path inFile = new Path(locateResource("ipoint.geojson").getPath());
  ...
  public void testReadFeatures() throws IOException {
    Path inFile = new Path(locateResource("/features.geojson").getPath());
  ...
  public void testResetLinestrings() throws IOException {
    Path inFile = new Path(locateResource("/linestrings.geojson").getPath());
  ...
  public void testReadAllFeatureTypes() throws IOException {
    Path inFile = new Path(locateResource("/allfeatures.geojson").getPath());
  ...
  public void testReadCompressedFile() throws IOException {
    Path inFile = new Path(locateResource("/allfeatures.geojson.bz2").getPath());
  ...
  public void testReadGeometryWithTypeComingLast() throws IOException {
    String geoJSONText = "{ \"coordinates\": [120, 3], \"type\": \"point\" }";
    JsonFactory jsonFactory = new JsonFactory();
    JsonParser parser = jsonFactory.createParser(geoJSONText);
    Geometry geom = GeoJSONFeatureReader.parseGeometry(parser);
    assertEquals("Point", geom.getGeometryType());
    assertEquals(120.0, geom.getCoordinate().getX());
  }
}