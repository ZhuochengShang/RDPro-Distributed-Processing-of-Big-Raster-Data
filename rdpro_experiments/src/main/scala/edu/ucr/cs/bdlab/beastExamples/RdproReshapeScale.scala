...
import edu.ucr.cs.bdlab.beast.cg.Reprojector.{TransformationInfo, findTransformationInfo, reprojectEnvelope, reprojectGeometry}
...
import edu.ucr.cs.bdlab.raptor.{GeoTiffWriter, PixelScale, RasterOperationsFocal}
...
@OperationMetadata(
  shortName = "rdproscale",
  description = "Computes reshape scale",
  inputArity = "1",
  outputArity = "1"
)
object RdproReshapeScale extends CLIOperation{
  ...
  def run(opts: BeastOptions, inputs: Array[String], outputs: Array[String], sc: SparkContext): Any ={
    val conf = new SparkConf().setAppName("Reshape Raster RDD")
    ...
    try {
      ...
      rasterWidth = Math.floor(Math.abs(maxX2 - minX1) / (minCellsizeX * 10)).toInt
      rasterHeight = Math.floor(Math.abs(maxY2 - minY1) / (minCellsizeY * 10)).toInt
      ...
      val DownScale_only:RasterRDD[Array[Int]] = RasterOperationsFocal.reshapeNN(rasterRDDFile,targetMetadata)

      DownScale_only.foreach(tile => tile.getPixelValue(tile.x1,tile.y1))

      GeoTiffWriter.saveAsGeoTiff(DownScale_only,outputFileCompat.getPath, Seq(GeoTiffWriter.WriteMode -> "distributed"))
      val endTime: Long = System.nanoTime()

      println("Total time of Reshape rescale of rasterRDD: " + (endTime - startReadTime) / 1E9)
      println("------ %%% FINISHED %%% ------")
    } finally {
      spark.stop()
    }
